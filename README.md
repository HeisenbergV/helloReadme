...
3. 开始开发之前，可以让所有功能都以事件形式触发，无论是test/web按钮/cli 等：（比如本来要自动定时或按一些策略采集数据，但改为页面操作）：这样的目的是可以快速确认功能是否符合要求，便于让ai调整。当点击页面发现功能符合要求后再让ai转为自动形式 这样转换对于ai来讲易如反掌
4. 一定要把源头的数据结构先规范好，比如此项目的源头数据是github项目信息，信息的字段类型结构都要明确好，否则ai可能会再实现不同模块的情况下乱用
5. 一步步实现全部功能，每次完成一个就开启新会话，保持上一个功能完善+文字说明（下一个会话能迅速理解）。一个对话只做一件事，善于用git，每次完成一次工作就保存一下，出问题后及时git checkout .
6. 功能的完成后也要让ai完成对这个功能的文字说明，便于不同ai或不同会话之间迅速理解
